...
couch_node_test(){
    ...
    docker run --rm -it \
           ...
           --name run_tests_sh jmarca/couch_node_tests:126 sh

}
# couch_node_test(){
#     del_stopped "couch_node_tests"
#     relies_on_network couchdb_nw
#     docker run --rm -it -u node -v ${PWD}:/usr/src/dev  -w /usr/src/dev --network=couchdb_nw --name couch_node_tests jmarca/couch_node_tests:current bash
# }
